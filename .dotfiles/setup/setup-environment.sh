...
if [ -f "$HOME/.dotfiles/install-fonts.sh" ]; then
  if prompt_yes "Install bundled FiraCode Nerd Fonts?"; then
    bash "$HOME/.dotfiles/install-fonts.sh"
  fi
else
  echo "Font installer not found at $HOME/.dotfiles/install-fonts.sh"
fi

echo

if [ -f "$HOME/.dotfiles/setup-github-ssh.sh" ]; then
  if prompt_yes "Run GitHub SSH setup and switch dotfiles remote to SSH?"; then
    DOTFILES_SSH_REPO="$SSH_REPO_URL" bash "$HOME/.dotfiles/setup-github-ssh.sh"
  fi
else
  echo "GitHub SSH setup script not found at $HOME/.dotfiles/setup-github-ssh.sh"
fi
